...
function mostSpokenLanguages(countries, num) 
{
  ...
  const temp = []
  let filternSort = []
  const language = countries.reduce((a, b) => a.concat(b.languages), [])
  for (i in language)
  {
    let tmp = {}
    tmp.country = language[i]
    tmp.count = language.reduce((a, b) =>
    {
      if (b === tmp.country) return a + 1
      else return a
    }, 0)
    temp.push(tmp)
  }
  filternSort = temp.filter((a, b, c) =>
  {
    return c.findIndex(p => p.country === a.country) === b
  }).sort((a, b) => {
...
function mostPopulatedCountries(countries, num)
{
  ...
  {
    let tmp = {}
    tmp.country = countries[i].name
    tmp.population = countries[i].population
    temp.push(tmp)
  }
