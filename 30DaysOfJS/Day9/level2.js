...
const sum1 = products.filter((a) => typeof a.price === 'number').map(a => a.price).reduce((a, b) => a + b)
// console.log(sum1)
const sum2 = products.reduce((a, b) => {
    if (typeof b.price === "number") return a + b.price
    else return a
}, 0)
// console.log(sum2)
function categorizeCountries(countries, pattern)
{
    return countries.filter((a) => a.name.includes(pattern))
}
const landCountries = categorizeCountries(countries, "land")
const iaCountries = categorizeCountries(countries, "ia")
const islandCountries = categorizeCountries(countries, "island")
const stanCountries = categorizeCountries(countries, "stan")
// console.log(landCountries)
// console.log(iaCountries)
// console.log(islandCountries)
// console.log(stanCountries)
function letterAndNumbers(countries)
{
    const result = []
    for (i in countries)
    {
        var tmp = {}
        tmp.letter = countries[i].name[0], 
        tmp.frequency = countries.reduce((a, b) =>
            {
                if (b.name[0] === tmp.letter) return a + 1;
                else return a
            }, 0)
        result.push(tmp)
    }
    return result.filter((a, b, c) =>
    {
        return c.findIndex(p => p.letter === a.letter) === b
    })
}
const test = letterAndNumbers(countries)
console.log(test)
...
{
    const result = []
    for (let i = countries.length; i > countries.length - 10; i--)
    {
        result.push(countries[i])
    }
    return result
}
console.log(test.reduce((a, b) => 
{
